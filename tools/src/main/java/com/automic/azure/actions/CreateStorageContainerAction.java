...
public class CreateStorageContainerAction extends AbstractAction {
	...
	private static final Logger LOGGER = LogManager
			.getLogger(CreateStorageContainerAction.class);
	...
	protected void initialize() {
		...
		this.storageAccount = new AzureStorageAccount(
				getOptionValue("storage"), getOptionValue("accesskey"));
		// container Name
		this.containerName = getOptionValue("containername");
		// access level of container
		if (getOptionValue("containeraccess") != null) {
			this.containerAccess = ContainerAccess
					.valueOf(getOptionValue("containeraccess"));
		}

		// authentication service
		this.authenticationService = new AzureStorageAuthenticationService(
	...
	protected void validateInputs() throws AzureException {
		...
		if (!Validator.checkNotEmpty(storageAccount.getPrimaryAccessKey())) {
			...
			throw new AzureException(
					ExceptionConstants.EMPTY_STORAGE_ACCESS_KEY);
		}
	...
	/**
	 * {@inheritDoc com.automic.azure.actions.AbstractAction#executeSpecific }.
	...
	protected ClientResponse executeSpecific(Client storageHttpClient)
		...
		// get URL
		WebResource resource = storageHttpClient.resource(
				this.storageAccount.blobURL()).path(containerName);
		...
		this.authenticationService.addStorageHttpHeaders("x-ms-version",
				getOptionValue("xmsversion"));
		this.authenticationService.addStorageHttpHeaders("x-ms-date",
				CommonUtil.getCurrentUTCDateForStorageService());
		// add query parameters
		this.authenticationService.addQueryParameter("restype", "container");
		// update URI
		String clientURIForSignature = "/"
				+ this.storageAccount.getAccountName() + "/" + containerName;
		this.authenticationService.setURIforSignature(clientURIForSignature);

		// set query parameters
		Map<String, String> queryParameters = this.authenticationService
				.getQueryParameters();
		for (String headerKey : queryParameters.keySet()) {
			resource = resource.queryParam(headerKey,
					queryParameters.get(headerKey));
		}

		WebResource.Builder builder = resource.getRequestBuilder();
		// set storage headers
		Map<String, String> storageHttpHeaders = this.authenticationService
				.getStorageHttpHeaders();
		// calculate Authorization header
		storageHttpHeaders.put("Authorization",
				this.authenticationService.createAuthorizationHeader());
		for (String headerKey : storageHttpHeaders.keySet()) {
			builder = builder.header(headerKey,
